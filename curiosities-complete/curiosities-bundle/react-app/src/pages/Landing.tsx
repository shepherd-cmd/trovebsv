...
import { TREASURY_IDENTITY } from "@/lib/metanet";
...
function RecentlyUploaded() {
  ...
  const { isConnected, splitPayment, userIdentity } = useMetanet();
  ...
  const handleUncover = async () => {
    ...
    if (!isConnected) {
      // Not connected — open identity/wallet connect
      setSelectedDoc(null);
      return;
    }
    setPaying(true);
    try {
      ...
      await splitPayment(Math.round(amountBsv * 1e8), selectedDoc.owner_paymail || TREASURY_IDENTITY, `Unlock: ${selectedDoc.title}`);
      // Record the unlock in Supabase (80/10/10 split)
      const ownerShare = amountBsv * 0.80;
      const platformShare = amountBsv * 0.10;
      const gorillaPoolShare = amountBsv - ownerShare - platformShare;
      await supabase.from('document_unlocks').insert({
        document_id: selectedDoc.id,
        owner_share: ownerShare,
        platform_share: platformShare,
        gorilla_pool_share: gorillaPoolShare,
        amount_paid: amountBsv,
      });
      // Update total_earnings on the document for leaderboard
      await supabase.rpc('increment_document_earnings', {
          ...
          >
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {UNLOCK_PRICE_SATS} sats · paid via your BSV wallet
              </p>
...
const FEATURE_CARDS = [
  {
    ...
    synopsis: "The moment you upload something, curIosities stamps it to the BSV blockchain — a public record that can never be altered, deleted, or faked. If someone ever asks \"is this genuine?\" — the blockchain answers for you. Permanently.",
  },
  {
    icon: Zap,
  ...
  },
  {
    ...
    synopsis: "Your upload lives on the blockchain, not on curIosities' servers. Even if curIosities disappeared tomorrow, your document, your record, and your royalty rights would still exist. It's yours — and your family's — for as long as the internet exists.",
  },
];
      ...
      <section className="py-16 px-4 relative overflow-hidden">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="space-y-4 text-muted-foreground text-base md:text-lg leading-relaxed mb-8">
            <p>D-Day maps. Resistance letters. Factory ledgers. Hand-drawn plans. The only surviving photos of forgotten regiments. Most of it will be lost the day the last person who remembers it is gone.</p>
            <p className="text-xl font-bold font-display brass-glow">curIosities changes that.</p>
