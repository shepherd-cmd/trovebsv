...
export const embedWatermark = (imageDataUrl: string, txid: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const message = `CURIOSITIES:${txid}`;
...
export const extractWatermark = (imageDataUrl: string): Promise<string | null> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // Support both current prefix and legacy 'TROVE:' prefix for backward compatibility
      if (message.startsWith('CURIOSITIES:')) {
        resolve(message.substring(12));
      } else if (message.startsWith('TROVE:')) {
